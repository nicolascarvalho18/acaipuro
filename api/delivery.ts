...
export default async function handler(req: any, res: any) {
  ...
  // GET: Handlers
  if (req.method === 'GET') {
    const type = req.query?.type || 'drivers';

    // 1. Listar Entregadores
    if (type === 'drivers' || !req.query?.type) {
      if (supabase) {
        try {
    ...
    // 2. Listar Atribuições / Corridas
    if (type === 'assign' || type === 'assignments') {
    ...
    // 3. Obter Localização do Entregador
    if (type === 'location') {
      const orderNumber = req.query?.orderNumber;
      const driverId = req.query?.driverId;
  ...
  if (req.method === 'POST') {
    try {
      ...
      // 6. Atualizar Localização GPS
      if (action === 'update_location' || (body?.latitude && body?.longitude)) {
      ...
      return res.status(500).json({ error: 'Erro ao processar', message: e?.message });
    }
  }
